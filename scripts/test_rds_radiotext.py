...
than the demodulator - no radio, no capture, no bits.
...
def group_2a(proto, ab, addr, chars):
...
print("\none corrupted character is not a new message")
p = RdsProtocol()
announce_rtplus(p)
send_rt(p, SONG)
send_rtplus(p, (4, 11, 9))
# A block the code "corrected" wrongly: segment 2 is " - D" and the D of
# "Dan" comes back as an X. Clearing on that one character would blank the
# display for the four seconds a refill takes.
group_2a(p, 0, 2, " - X")
